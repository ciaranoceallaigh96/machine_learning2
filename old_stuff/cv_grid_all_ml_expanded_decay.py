...
param_grid = {'decay_rate':[0.75], 'max_norm_reg':[True], 'lr_schedule':[True], 'network_shape':['funnel'], 'epochs' : [75],'batch_size' : [32],'learning_rate' : [0.001],'HP_L1_REG' : [1e-4],'HP_L2_REG' : [1e-8], 'kernel_initializer' : ['he_normal'],'activation' : ['relu'],'HP_NUM_HIDDEN_LAYERS' : [3],'units' : [16], 'rate' : [0.65],'HP_OPTIMIZER' : ['Adamax']}
...
def build_nn(decay_rate, max_norm_reg, lr_schedule, HP_OPTIMIZER, HP_NUM_HIDDEN_LAYERS, units, activation, learning_rate, HP_L1_REG, HP_L2_REG, rate, kernel_initializer, network_shape):
	opt = HP_OPTIMIZER
	...
	model = Sequential()
	input_shape = (x_train.shape[1],) if snps == 'shuf' else (16,)
	model.add(Dense(units=units, activation=activation, kernel_regularizer=reg, input_shape=input_shape, kernel_constraint=max_norm))
	if rate != 0:
	...
	for i in range(HP_NUM_HIDDEN_LAYERS-1):
		if network_shape == 'funnel':
			...
			if long_funnel_count >= 1: #two wide layers (inclduing previous first layer)
				units=int(units*0.666)
			long_funnel_count += 1
		model.add(Dense(units=units, activation=activation, kernel_regularizer=reg, kernel_initializer=kernel_initializer, kernel_constraint=max_norm))
		if rate != 0:
			model.add(Dropout(rate=rate))
	if binary == 'True' :
...
def conv_model(max_norm_reg, lr_schedule, HP_OPTIMIZER, HP_NUM_HIDDEN_LAYERS, units, activation, learning_rate, HP_L1_REG, HP_L2_REG, rate, kernel_initializer,strides,pool,filters,kernel, network_shape):
        opt = HP_OPTIMIZER
        ...
        model.add(Conv1D(filters=filters, strides=strides, input_shape=input_shape,  padding='same',data_format='channels_last', activation=activation, kernel_regularizer=reg, kernel_constraint=max_norm, kernel_initializer=kernel_initializer, kernel_size=kernel))
        ...
        for i in range(HP_NUM_HIDDEN_LAYERS-1):
                if network_shape == 'funnel':
                        ...
                        if long_funnel_count >= 1: #two wide layers (inclduing previous first layer)
                                units=int(units*0.666)
                        long_funnel_count += 1
                model.add(Conv1D(filters=filters, strides=strides, activation=activation,  padding='same',data_format='channels_last', kernel_regularizer=reg, kernel_initializer=kernel_initializer, kernel_constraint=max_norm, kernel_size=kernel))
                model.add(tf.keras.layers.MaxPool1D(pool_size=pool, strides=strides,padding='same', data_format='channels_last'))
        model.add(Flatten())
        if binary == 'True':
                model.add(Dense(1, activation='sigmoid'))
