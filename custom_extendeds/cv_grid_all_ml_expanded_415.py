...
c_param = [0.02, 0.01, 0.1,0.002]#[2e-2,2e-4,2e-8, 1,int(2e+2),int(2e+4),int(2e+8)] #can be negative #We found that trying exponentially growing sequences of C and γ is a practical method to identify good parameters https://www.csie.ntu.edu.tw/~cjlin/papers/guide/guide.pdf
rbf_c = [1,int(2e+2),int(2e+4),int(2e+8)] #[2e-2,2e-4,2e-8, 1,int(2e+2),int(2e+4),int(2e+8)]
gamma_param = [0.002,0.2,0.01]#not0.5 #ValueError: gamma < 0
epsilon_param = [2e-3,1,0]#[2e-5,2e-3,1,0]
...
alpha = [0.01,0.05,0.005] #[0.0001, 0.001, 0.01, 0.1, 1, 10, 100, -1, -10, -100]
max_iter=[1000,3000]
ridge_alpha = [10, 100,1000] #[0.0001, 0.001, 0.01, 0.1, 1, 10, 100, -1, -10, -100]
tolerance=[1e-3,1e-5,1e-1]
...
max_depth = [50,100,1000]#not 1 or 10
...
min_samples_split = [2, 10, 100]#[2, 10, 100, 1000] # Minimum number of samples required at each leaf node
...
min_samples_leaf = [1] #[1,2, 10, 100, 1000]
bootstrap = [False, True]
max_leaf_nodes = [10, 100, 500] #; max_leaf_nodes.append(x_train.shape[0])
max_samples = [0.5, 0.9, 0.75]#[0.5, 0.9, 0.1, 0.01]
...
#nn not ftrl or adam
param_grid = {'network_shape':['brick', 'funnel','long_funnel'], 'epochs' : [50,100,200],'batch_size' : [16,32, 128],'learning_rate' : [0.01, 0.001, 0.0001, 0.00001],'HP_L1_REG' : [1e-5,1e-6,1e-4, 1e-2, 0.1, 1e-3],'HP_L2_REG' : [1e-8, 1e-3, 1e-1, float(0)], 'kernel_initializer' : ['glorot_uniform', 'glorot_normal', 'random_normal', 'random_uniform', 'he_uniform', 'he_normal'],'activation' : ['tanh', 'relu', 'elu'],'HP_NUM_HIDDEN_LAYERS' : [2,3,5],'units' : [200, 100,1000], 'rate' : [float(0), 0.1, 0.3],'HP_OPTIMIZER' : ['RMSprop', 'Adadelta', 'Adamax', 'Adagrad', 'SGD']}
