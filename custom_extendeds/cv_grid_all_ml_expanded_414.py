...
def make_param_box_plot(goal_dict, time_dict, analysis, stability_dict=None): #example goal dict = {'alpha' : {0.1 : [0.3, 0.5, 0.4], 1 : [0, 0.1, 0.2]}, 'beta' : {0.1 : [0.5, 0.5, 0.45, 1 : [0.8, 0.7, 0.7]}}
	...
	if stability_dict is not None:
		...
		for i in stability_dict:
			...
			stability_dict[i] = {str(key): value for key, value in keys_values}
			plt.bar(*zip(*stability_dict[i].items()))
...
#			plt.boxplot(ordered_stability_items.values(), bootstrap=None,showmeans=False, meanline=False, notch=True,labels=ordered_stability_items.keys()) #box
#			plt.xlabel(str(param).upper(), fontsize=10, fontweight='bold')
...
c_param = [2e-2,2e-4, 1,int(2e+2),int(2e+4),int(2e+8)] #can be negative #We found that trying exponentially growing sequences of C and γ is a practical method to identify good parameters https://www.csie.ntu.edu.tw/~cjlin/papers/guide/guide.pdf
rbf_c = [1,int(2e+2),int(2e+4),int(2e+8)] #[2e-2,2e-4,2e-8, 1,int(2e+2),int(2e+4),int(2e+8)]
gamma_param = [0.2,0.5, 0.9] #ValueError: gamma < 0 #[0.002,0.2,0.5,0.01]
...
max_iter=[1000,3000]
ridge_alpha = [100,1000,500] # [0.0001, 0.001, 0.01, 0.1, 1, 10, 100, -1, -10, -100]
tolerance=[1e-3,1e-5,1e-1]
...
min_samples_leaf = [1,2, 10] # [1,2, 10, 100, 1000]
...
#nn not ftrl or addelta or sgd
#nn l1 not 0.1
#param_grid = {'HP_L1_REG': [0.001], 'learning_rate': [1e-05], 'HP_L2_REG': [0.1], 'activation': ['elu'], 'HP_NUM_HIDDEN_LAYERS': [3], 'batch_size': [16], 'kernel_initializer': ['he_normal'], 'network_shape': ['brick'], 'epochs': [100], 'rate': [0.1], 'HP_OPTIMIZER': ['Adam'], 'units': [100]}
param_grid = {'network_shape':['funnel','long_funnel'], 'epochs' : [100,200],'batch_size' : [128],'learning_rate' : [0.001, 0.0001],'HP_L1_REG' : [1e-2],'HP_L2_REG' : [1e-8, 1e-3, float(0)], 'kernel_initializer' : ['he_normal'],'activation' : ['relu'],'HP_NUM_HIDDEN_LAYERS' : [3],'units' : [200, 100], 'rate' : [float(0), 0.3],'HP_OPTIMIZER' : ['Adamax', 'Adam']}
#param_grid = {'network_shape':['brick', 'funnel','long_funnel'], 'epochs' : [50,100,200],'batch_size' : [16,32, 128],'learning_rate' : [0.01, 0.001, 0.0001, 0.00001],'HP_L1_REG' : [1e-5,1e-6,1e-4, 1e-2, 1e-3],'HP_L2_REG' : [1e-8, 1e-3, 1e-1, float(0)], 'kernel_initializer' : ['glorot_uniform', 'glorot_normal', 'random_normal', 'random_uniform', 'he_uniform', 'he_normal'],'activation' : ['tanh', 'relu', 'elu'],'HP_NUM_HIDDEN_LAYERS' : [2,3,5],'units' : [200, 100,1000], 'rate' : [float(0), 0.1, 0.3],'HP_OPTIMIZER' : ['RMSprop', 'Adamax', 'Adam', 'Adagrad']}
