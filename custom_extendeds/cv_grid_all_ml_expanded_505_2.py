...
c_param = [2e-2, 1,int(2e+2),int(2e+4),int(2e+8)] #check stability
rbf_c = [1,int(2e+2),int(2e+4),int(2e+8)] #at least one
gamma_param = [0.002,0.01]  
epsilon_param = [2e-5,2e-3,1,0] ##all the same, check stability
loss_param = ['epsilon_insensitive', 'squared_epsilon_insensitive'] #no diff check stability
kernel_param = ['rbf', 'sigmoid'] #check stability
tolerance=[1e-3,1e-5,1e-1]
shrinking=[True,False] #check stability
...
tolerance=[1e-3,1e-5,1e-1] #no diff, check stability
selection=['cyclic','random'] #check stability
...
n_estimators = [10,100,1000] #check stability
max_features = ['sqrt', 'log2'] #check stability
max_depth = [10, 50,100]
max_depth.append(None) 

min_samples_split = [2, 10] 

min_samples_leaf = [1,2] #check stability
bootstrap = [False, True] #check stability
max_leaf_nodes = [10, 100, 500] 
max_samples = [0.5, 0.9, 0.1, 0.01] #check stability
...
param_grid = {'network_shape':['brick', 'funnel','long_funnel'], 'epochs' : [50,100,200],'batch_size' : [16,32, 128],'learning_rate' : [0.01, 0.001, 0.0001, 0.00001],'HP_L1_REG' : [1e-5,1e-6,1e-4, 1e-2, 1e-3],'HP_L2_REG' : [1e-8, 1e-3, 1e-1], 'kernel_initializer' : ['glorot_uniform', 'glorot_normal', 'random_normal', 'random_uniform', 'he_uniform', 'he_normal'],'activation' : ['tanh', 'relu', 'elu'],'HP_NUM_HIDDEN_LAYERS' : [2,3,5],'units' : [200, 100,1000], 'rate' : [float(0), 0.1, 0.3],'HP_OPTIMIZER' : ['RMSprop', 'Adadelta', 'Adamax', 'Adam', 'Adagrad', 'SGD']}
