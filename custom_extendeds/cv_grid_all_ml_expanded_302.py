...
c_param = [1e-2,1e-3, 0.5] #can be negative #We found that trying exponentially growing sequences of C and γ is a practical method to identify good parameters https://www.csie.ntu.edu.tw/~cjlin/papers/guide/guide.pdf
rbg_c = [1, 0.5, 1.5]
gamma_param = [0.001, 0.0001, 0.00001] #ValueError: gamma < 0
epsilon_param = [1,10,100]
...
svm_random_grid = {'gamma':gamma_param, 'C':rbg_c,'kernel':kernel_param, "degree":degree, 'epsilon':epsilon_param, "shrinking":shrinking,"tol":tolerance,"cache_size":cache_size}
...
max_iter=[1000]
ridge_alpha = [10, 100, 500, 1000]
tolerance=[1e-3]
selection=['cyclic']# default=’cyclic’
...
n_estimators = [2000, 5000] # Number of features to consider at every split
...
param_grid = {'network_shape':['funnel'], 'epochs' : [50,100,200],'batch_size' : [32],'learning_rate' : [0.01, 0.001],'HP_L1_REG' : [1e-2, 0.1],'HP_L2_REG' : [1e-1, float(0)], 'kernel_initializer' : ['glorot_uniform', 'glorot_normal', 'random_normal', 'random_uniform', 'he_uniform', 'he_normal'],'activation' : ['tanh', 'relu', 'elu'],'HP_NUM_HIDDEN_LAYERS' : [2,3],'units' : [200, 100,1000], 'rate' : [float(0), 0.05],'HP_OPTIMIZER' : ['RMSprop', 'Adamax', 'Adam', 'Adagrad']}
...
cnn_param_grid = {'network_shape':['brick', 'funnel','long_funnel'], 'epochs':[100, 50],'batch_size' : [16,64,128], 'learning_rate' : [0.01, 0.0001, 0.001],'HP_L1_REG' : [0],'HP_L2_REG' : [0, 0.001,0.00001],'kernel_initializer' : ['glorot_normal', 'glorot_uniform', 'he_uniform', 'random_normal', 'random_uniform', 'he_normal'],'activation' : ['tanh', 'relu', 'elu'],'HP_NUM_HIDDEN_LAYERS' : [2,3, 5,10],'units' : [100,200,1000], 'rate' : [float(0), 0.1, 0.5],'HP_OPTIMIZER' : ['Adamax', 'Adam'], 'filters':[1,5, 10],'strides':[1,2, 5, 10],'pool':[1,2,10],'kernel':[1,2,10]}
