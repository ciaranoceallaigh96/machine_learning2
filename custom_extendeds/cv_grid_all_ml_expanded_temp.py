print("Adjusting based on results from cv_grid_all_ml_150_top_100_ft10_v2.txt")
...
c_param = [1e-1, 1,2,10,75,200] #can be negative #We found that trying exponentially growing sequences of C and γ is a practical method to identify good parameters https://www.csie.ntu.edu.tw/~cjlin/papers/guide/guide.pdf
c_param_rbg = [1,2,10,75,200]
...
svm_random_grid = {'gamma':gamma_param, 'C':c_param_rbg,'kernel':kernel_param, "degree":degree, 'epsilon':epsilon_param, "shrinking":shrinking,"tol":tolerance,"cache_size":cache_size}
...
alpha = [0.001, 0.01, 0.1, 0.2, 0.05]
max_iter=[1000,3000]
ridge_alpha = [1, 200, 100, -100, -200, 500]
tolerance=[1e-3,1e-5,1e-1]
...
n_estimators = [1000, 3000, 5000] # Number of features to consider at every split
...
min_samples_split = [2, 10, 20] # Minimum number of samples required at each leaf node
...
max_leaf_nodes = [10, 2, 5] #; max_leaf_nodes.append(x_train.shape[0]) #cant be 1 
...
param_grid = {'network_shape':['brick', 'funnel','long_funnel'], 'epochs' : [50,100,200],'batch_size' : [16,32, 128],'learning_rate' : [0.01, 0.001, 0.0001, 0.00001, 0.1],'HP_L1_REG' : [1e-5,1e-6,1e-4, 1e-2, 0.1, 1e-3],'HP_L2_REG' : [1e-8, 1e-3, 1e-1, float(0)], 'kernel_initializer' : ['glorot_uniform', 'glorot_normal', 'random_normal', 'random_uniform', 'he_uniform', 'he_normal'],'activation' : ['tanh', 'relu', 'elu'],'HP_NUM_HIDDEN_LAYERS' : [2,3,5, 8],'units' : [200, 100,1000], 'rate' : [float(0), 0.1, 0.3],'HP_OPTIMIZER' : ['RMSprop', 'Adamax', 'Adam', 'Adagrad', 'SGD']}
...
cnn_param_grid = {'network_shape':['brick', 'funnel','long_funnel'], 'epochs':[100, 50],'batch_size' : [16,64,128], 'learning_rate' : [0.01, 0.0001, 0.001, 0.1],'HP_L1_REG' : [0.001, 0.0001,0.00001,0],'HP_L2_REG' : [0, 0.001,0.00001],'kernel_initializer' : ['glorot_normal', 'glorot_uniform', 'he_uniform', 'random_normal', 'random_uniform', 'he_normal'],'activation' : ['tanh', 'relu', 'elu'],'HP_NUM_HIDDEN_LAYERS' : [2,3, 5, 8],'units' : [100,200,1000], 'rate' : [float(0), 0.1, 0.5],'HP_OPTIMIZER' : ['SGD','Ftrl', 'RMSprop', 'Adadelta', 'Adamax', 'Adam', 'Adagrad'], 'filters':[1,5],'strides':[1,2,3],'pool':[1,2,3],'kernel':[1,2,3]}
