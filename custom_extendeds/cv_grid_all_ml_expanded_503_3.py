...
c_param = [2e-2, 1,int(2e+2),int(2e+4)] #can be negative #We found that trying exponentially growing sequences of C and γ is a practical method to identify good parameters https://www.csie.ntu.edu.tw/~cjlin/papers/guide/guide.pdf
rbf_c = [1,int(2e+2),int(2e+4),int(2e+8)]
gamma_param = [0.001,0.005] #ValueError: gamma < 0
epsilon_param = [1]
...
min_samples_leaf = [1]
bootstrap = [False]
max_leaf_nodes = [10, 100] #; max_leaf_nodes.append(x_train.shape[0])
...
param_grid = {'network_shape':['brick', 'funnel','long_funnel'], 'epochs' : [50,100,200],'batch_size' : [16, 128],'learning_rate' : [0.001],'HP_L1_REG' : [1e-5,1e-6,1e-4, 1e-3],'HP_L2_REG' : [1e-8, 1e-3, float(0)], 'kernel_initializer' : ['random_normal', 'random_uniform'],'activation' : ['relu'],'HP_NUM_HIDDEN_LAYERS' : [2,3],'units' : [200, 100,1000], 'rate' : [float(0), 0.1, 0.3],'HP_OPTIMIZER' : ['Adamax', 'Adam']}
...
cnn_param_grid = {'network_shape':['brick'], 'epochs':[50],'batch_size' : [16,64,128], 'learning_rate' : [0.001],'HP_L1_REG' : [0],'HP_L2_REG' : [0,0.00001],'kernel_initializer' : ['glorot_normal', 'glorot_uniform', 'he_uniform', 'he_normal'],'activation' : ['tanh'],'HP_NUM_HIDDEN_LAYERS' : [2, 5],'units' : [1000], 'rate' : [0.1],'HP_OPTIMIZER' : ['RMSprop', 'Adamax', 'Adam'], 'filters':[5],'strides':[1],'pool':[2],'kernel':[2,3]}
