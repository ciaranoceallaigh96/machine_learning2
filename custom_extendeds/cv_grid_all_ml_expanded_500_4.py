...
c_param = [1,20000] #can be negative #We found that trying exponentially growing sequences of C and γ is a practical method to identify good parameters https://www.csie.ntu.edu.tw/~cjlin/papers/guide/guide.pdf
rbf_c = [1,int(2e+2),int(2e+4),int(2e+8)]
gamma_param = [0.0005,0.001] #ValueError: gamma < 0
epsilon_param = [2e-5,2e-3,0]
...
ridge_alpha = [-10, -100]
tolerance=[1e-3,1e-5,1e-1]
selection=['random']# default=’cyclic’
...
param_grid = {'network_shape':['funnel','long_funnel'], 'epochs' : [50,100],'batch_size' : [16,32],'learning_rate' : [0.001],'HP_L1_REG' : [1e-5,1e-6,1e-4],'HP_L2_REG' : [1e-8, 1e-3, 1e-1, float(0)], 'kernel_initializer' : ['glorot_uniform','random_uniform', 'he_normal'],'activation' : ['tanh', 'relu'],'HP_NUM_HIDDEN_LAYERS' : [2,3],'units' : [200, 100,1000], 'rate' : [float(0), 0.1, 0.3],'HP_OPTIMIZER' : ['Adamax']}
...
def build_nn(HP_OPTIMIZER, HP_NUM_HIDDEN_LAYERS, units, activation, learning_rate, HP_L1_REG, HP_L2_REG, rate, kernel_initializer, network_shape):
	...
	model.add(Dense(units=units, activation=activation, kernel_regularizer=reg, kernel_initializer=kernel_initializer, input_shape=input_shape))
	if rate != 0:
...
cnn_param_grid = {'network_shape':['funnel','long_funnel'], 'epochs':[100, 50],'batch_size' : [16,64], 'learning_rate' : [0.01, 0.0001, 0.001],'HP_L1_REG' : [0.00001,0],'HP_L2_REG' : [0, 0.001,0.00001],'kernel_initializer' : ['glorot_normal', 'glorot_uniform'],'activation' : ['tanh'],'HP_NUM_HIDDEN_LAYERS' : [2],'units' : [100,200,1000], 'rate' : [float(0), 0.1, 0.5],'HP_OPTIMIZER' : ['Adamax', 'Adam'], 'filters':[1,5],'strides':[1],'pool':[1,2,3],'kernel':[2,3]}
