...
rbf_c = [1,2, 5]
#rbf_c = [1,2, 5, int(2e+2)] 
 #can be negative #We found that trying exponentially growing sequences of C and γ is a practical method to identify good parameters https://www.csie.ntu.edu.tw/~cjlin/papers/guide/guide.pdf
c_param = [2e-2, 1,0.5]
gamma_param = [0.002,0.0005]
#gamma_param = [0.002,0.01]  #ValueError: gamma < 0
epsilon_param = [0.1, 0.5, 1]
#epsilon_param = [2e-5,2e-3,0.1,0]
...
svm_random_grid2 = {'C' : rbf_c, 'loss':loss_param, 'epsilon':epsilon_param}
...
n_estimators = [10,100,1000, 5000] # Number of features to consider at every split
...
min_samples_split = [2, 10, 50] # Minimum number of samples required at each leaf node
...
param_grid = {'network_shape':['brick', 'funnel','long_funnel'], 'epochs' : [50,100,200],'batch_size' : [16,32, 128],'learning_rate' : [0.01, 0.001, 0.0001],'HP_L1_REG' : [1e-5,1e-6,1e-4, 1e-3],'HP_L2_REG' : [1e-8, 1e-3, 1e-1, float(0)], 'kernel_initializer' : ['he_normal'],'activation' : ['tanh', 'relu', 'elu'],'HP_NUM_HIDDEN_LAYERS' : [2,3,5],'units' : [200, 100,1000], 'rate' : [float(0), 0.1, 0.3],'HP_OPTIMIZER' : ['Adamax', 'Adam']}
...
cnn_param_grid = {'network_shape':['brick', 'funnel','long_funnel'], 'epochs':[100, 50],'batch_size' : [16,64,128], 'learning_rate' : [0.01, 0.0001, 0.001],'HP_L1_REG' : [0.001, 0.0001,0.00001,0],'HP_L2_REG' : [0, 0.001,0.00001],'kernel_initializer' : ['glorot_normal', 'glorot_uniform', 'he_uniform', 'random_normal', 'random_uniform', 'he_normal'],'activation' : ['tanh', 'relu', 'elu'],'HP_NUM_HIDDEN_LAYERS' : [2,3, 5],'units' : [100,200,1000], 'rate' : [float(0), 0.1, 0.5],'HP_OPTIMIZER' : ['SGD', 'RMSprop', 'Adamax', 'Adam', 'Adagrad'], 'filters':[1,50],'strides':[1],'pool':[1,10],'kernel':[1,10]}
