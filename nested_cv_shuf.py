...
def bash_script(train_index, test_index, train_names, test_names, outer_count, inner_count):
        ...
        with open("name_vector_test.txt", 'w') as f:
            for item in test_names:
                f.write("%s %s\n" % (item, item))        
        subprocess.run(["/external_storage/ciaran/machine_learning2/bash_script_shuf.sh", str(outer_count), str(inner_count)], shell=True) 
        time.sleep(60)
