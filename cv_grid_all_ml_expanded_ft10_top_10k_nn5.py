...
param_grid = {'network_shape':['funnel','long_funnel'], 'epochs' : [50,100,200],'batch_size' : [128, 256, 512],'learning_rate' : [0.01, 0.001],'HP_L1_REG' : [1e-5,1e-6],'HP_L2_REG' : [1e-8, 1e-9], 'kernel_initializer' : ['he_normal'],'activation' : ['relu'],'HP_NUM_HIDDEN_LAYERS' : [8, 10, 15, 20],'units' : [1000, 2000, 5000], 'rate' : [float(0), 0.1, 0.3],'HP_OPTIMIZER' : ['Adamax']}
...
callback = tf.keras.callbacks.EarlyStopping(monitor='coeff_determination', patience=5, mode='max', min_delta=0.2) #min above 0

if binary == 'True': #overwrite variables
	dependencies = {'auc':tf.keras.metrics.AUC}
	METRIC_ACCURACY = tf.keras.metrics.AUC
	callback = tf.keras.callbacks.EarlyStopping(monitor='auc', patience=10, mode='max', min_delta=0.1) #min above 0
...
def build_nn(HP_OPTIMIZER, HP_NUM_HIDDEN_LAYERS, units, activation, learning_rate, HP_L1_REG, HP_L2_REG, rate, kernel_initializer, network_shape):
	...
	if binary == 'True' :
		...
		model.compile(loss='mean_squared_error',metrics=['accuracy', 'mse', coeff_determination],optimizer=chosen_opt(learning_rate=learning_rate))
